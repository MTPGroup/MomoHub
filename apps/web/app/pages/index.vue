...
<template>
  <UPageSection class="overflow-hidden">
    ...
    <div class="absolute inset-0 -z-10">
      <div
        class="absolute top-[-20%] left-[-10%] h-125 w-125 rounded-full bg-(--ui-color-primary)/10 blur-3xl"
      />
      <div
        class="absolute bottom-[-20%] right-[-10%] h-100 w-100 rounded-full bg-(--ui-color-primary)/5 blur-3xl"
      />
    </div>

    <UContainer class="py-24 lg:py-32 text-center">
      <h1
        class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-gray-900 dark:text-white"
      >
        发现无限可能的
        <span
          class="bg-linear-to-r from-(--ui-color-primary) to-emerald-400 bg-clip-text text-transparent"
        >
          AI 角色
        </span>
      </h1>
      <p
        class="mt-6 text-lg sm:text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto"
      >
      ...
      <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div class="flex flex-col items-center gap-4 py-2">
            <div
              class="flex h-14 w-14 items-center justify-center rounded-full bg-(--ui-color-primary)/10"
            >
              <UIcon :name="feature.icon" class="text-2xl bg-primary" />
            </div>
